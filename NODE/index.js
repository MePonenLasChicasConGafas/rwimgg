...
async function main() {
    ...
    console.log(image)
    console.log(`\u{2500}`.repeat(process.stdout.columns)) //hr
    console.log('[!] Sauce/ Image Link: ' + chalk.blue(link)) //link

    rl.question(chalk.yellow('[?] Save image? [y/N] '), input => {
        if (input == 'y'??'Y') {
            https.get(link, resp => resp.pipe(fs.createWriteStream(`./saved/${filename}`)));
            console.log(chalk.green('[✓] File saved successfully in ./saved/ as '+filename))
        
        } else if (input == 'n'??'N') {
            rl.close()
        } else {
            console.log('Strange input; exited')
            process.exit()
        }
